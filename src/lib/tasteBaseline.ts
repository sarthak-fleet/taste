...
function extractEvidenceFeatures(variant: TasteBaselineVariant): VariantEvidenceFeatures {
  ...
  for (const artifact of variant.artifacts) {
    ...
    maxFirstSectionHeightRatio = Math.max(
      maxFirstSectionHeightRatio,
      metrics.page.firstSectionHeightRatio ?? 0
    );
    minVisibleActionCount = Math.min(minVisibleActionCount, metrics.page.visibleActionCount);
    if (artifact.viewport === 'mobile')
      mobileRiskScore = Math.max(mobileRiskScore, metrics.risk.score);
    if (artifact.viewport === 'desktop')
      desktopRiskScore = Math.max(desktopRiskScore, metrics.risk.score);
  }
...
function scoreVariant(features: VariantEvidenceFeatures): DimensionScores {
  ...
  return {
    ...
    firstActionClarity: clampScore(
      4.2 - missingActionPenalty - heroPenalty - clippedPenalty * 0.15
    ),
    perceivedValue: clampScore(4.1 - riskPenalty - failedImagePenalty * 0.6 - desktopPenalty * 0.4),
    friction: clampScore(
      1.6 + riskPenalty * 2.1 + overflowPenalty + mobilePenalty + clippedPenalty * 0.4
    ),
    differentiation: clampScore(3.2 - riskPenalty * 0.25),
    completionConfidence: clampScore(
      4.1 - mobilePenalty - failedImagePenalty - overflowPenalty * 0.5
    ),
    conversionIntent: clampScore(
      4.0 - missingActionPenalty - heroPenalty * 0.5 - riskPenalty * 0.7
    ),
  };
}

function scoreTasteCriteria(
  scores: DimensionScores,
  features: VariantEvidenceFeatures
): Partial<Record<TasteCriterion, number>> {
  const readability = clampScore(
    scores.clarity - Math.min(0.8, features.totalClippedTextCandidates * 0.08)
  );

  return {
    typography: readability,
    layoutHierarchy: clampScore(
      (scores.clarity + scores.firstActionClarity + scores.perceivedValue) / 3
    ),
    spacing: clampScore(
      5 - Math.min(4, features.highestRiskScore / 22 + features.totalClippedTextCandidates * 0.08)
    ),
    ...
    readability,
    mobileFit: clampScore(
      5 -
        Math.min(4, features.mobileRiskScore / 20 + (features.maxHorizontalOverflow > 2 ? 0.7 : 0))
    ),
...
export function runTasteMechanicalBaselineForVariants(params: {
  ...
  const ranked = [...initialOutputs].sort(
    (a, b) =>
      dimensionToOverallScore(b.scores as DimensionScores) -
      dimensionToOverallScore(a.scores as DimensionScores)
  );
