...
export function generateReport(input: ReportInput): ReportContent {
  ...
  return {
    executiveRecommendation: {
      ...
      reason: whyWinnerWon[0] ?? `Highest overall score (${winner.overallScore}) across evaluators.`,
    ...
    decisionMemory: {
      ...
      assumption: input.study.contextConcerns || `Variant ${winner.variantLabel} best matches ${input.study.targetUserRole} needs.`,
...
function buildWhyWinnerWon(
  ...
  if (winner.scores.targetUser >= 4) {
    reasons.push(`Target users (${input.study.targetUserRole}) ranked Variant ${winner.variantLabel} highest for clarity and intent.`);
  }
  ...
  if (winner.scores.agent >= 4) {
    reasons.push(`AI agent panel consistently flagged ${winner.variantLabel} for strong first-action clarity.`);
  }
  if (input.humanQuotes.length === 0 && winner.scores.agent >= 3.5) {
    reasons.push(`Agent-first ranking: ${winner.variantLabel} led across specialized evaluator agents.`);
  }
...
function buildWinnerWeaknesses(
  ...
  if (winner.scores.targetUser < 4 && runnerUp && runnerUp.scores.targetUser > winner.scores.targetUser) {
...
function buildAgentFindings(
  ...
  for (const v of variants) {
    ...
    const avgRank =
      runs.length > 0
        ? runs.reduce((s, r) => s + r.prediction.predictedRank, 0) / runs.length
        : 99;
    if (avgRank <= 2) {
      const topFinding = runs.flatMap((r) => r.findings).find((f) => f.severity !== "high");
      consensus.push(
        `Variant ${v.label} is strong for ${topFinding?.type ?? "overall quality"}.`,
      );
    }
  }
...
function buildTradeoff(winner: VariantRanking, runnerUp: VariantRanking | undefined): string {
  ...
  if (winner.scores.targetUser > runnerUp.scores.targetUser && runnerUp.scores.agent > winner.scores.agent) {
