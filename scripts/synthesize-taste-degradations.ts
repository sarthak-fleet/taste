...
import type {
  CaptureIssue,
  MechanicalMetrics,
  TasteCaptureManifest,
} from '../src/lib/visualEvidence.ts';
...
const DEGRADATIONS: DegradationKind[] = [
  'clipped',
  'contrast',
  'hero',
  'mobile_overflow',
...
function parseArgs(argv: string[]): CliArgs {
  ...
  return {
    ...
    count: Math.max(
      1,
      Math.min(DEGRADATIONS.length, Number.isFinite(count) ? count : DEGRADATIONS.length)
    ),
  };
}
...
function degradeMetrics(metrics: MechanicalMetrics, kind: DegradationKind): MechanicalMetrics {
  ...
  if (kind === 'clipped') {
    next.issues.clippedText.push(
      syntheticIssue(kind, next),
      syntheticIssue(kind, next),
      syntheticIssue(kind, next)
    );
  } else if (kind === 'contrast') {
    next.issues.lowContrastText.push(
      syntheticIssue(kind, next),
      syntheticIssue(kind, next),
      syntheticIssue(kind, next)
    );
...
async function main() {
  ...
  for (const kind of chosen) {
    ...
    const degradedManifestPath = path.join(
      args.outDir,
      `${slugify(args.label)}-${kind}-degraded-manifest.json`
    );
