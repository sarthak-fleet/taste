...
async function collectMetrics(page, url, viewport, loadMs, capturedAt) {
  const raw = await page.evaluate(({ url, viewport, loadMs, capturedAt }) => {
    ...
    const visibleActionCount = [...document.querySelectorAll("a[href],button,input[type='submit']")]
      .filter((el) => {
...
const risks = manifest.artifacts.map((a) => `${a.viewport}:${a.metrics.risk.level}/${a.metrics.risk.score}`).join(", ");
