...
function validateQueue(queue: CaptureQueue) {
  ...
  for (const job of queue.jobs ?? []) {
    ...
    if (!SOURCE_KINDS.has(sourceKind))
      errors.push(`Job ${job.id || '(missing id)'} has invalid sourceKind ${sourceKind}`);
    for (const side of ['a', 'b'] as const) {
      ...
      if (!validateUrl(variant.url))
        errors.push(`Job ${job.id || '(missing id)'} has invalid ${side}.url`);
      urls.add(variant.url);
    }
    if (job.a?.url && job.b?.url && job.a.url === job.b.url)
      errors.push(`Job ${job.id} compares a URL to itself`);
  }
...
async function main() {
  ...
  const urls = [
    ...new Set(
      (queue.jobs ?? []).flatMap((job) => [job.a?.url, job.b?.url].filter(Boolean) as string[])
    ),
  ];
