...
export function canRunTasteVlmJudge(config: TasteVlmConfig, variants: TasteBaselineVariant[]) {
  return (
    Boolean(config.apiBase && config.apiKey && config.model) &&
    variants.filter((variant) => imageUrlsForVariant(variant).length > 0).length >= 2
  );
}
...
function buildPrompt(params: {
  ...
  const variantText = params.variants
    .map((variant) => {
      const desktop = variant.artifacts.find(
        (artifact) => artifact.viewport === 'desktop'
      )?.metrics;
...
export async function runTasteVlmJudgeForVariants(params: {
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are Taste, a strict website/product design preference judge. Return only JSON.',
        },
        {
          role: 'user',
  ...
  const overallWinnerVariantId =
    ...
    params.variants.some((variant) => variant.id === json.overallWinnerVariantId)
      ? json.overallWinnerVariantId
